 ...
 * --------------------
 * Channel 13 is allocated on Linux Devices to act as a remote shell. Refer to the Remote TTY protocol for more
 * information.
 *
 *
 * Control Interface Channel
 * -------------------------
 * Channel 14 is allocated as a general control interface for the client on the network. This follows the Register
 * Mapped Protocol (except for bootloader request). The heartbeat is used to determine which mode to attempt to
 * contact the device with.
 *
 ...
 *   If Client Heartbeat:Mode = Linux Mode
 *      Follows Linux Protocol
 *
 *
 * Heartbeat Channel
 * -----------------
 * Channel 15 (all 1s) is reserved for heartbeat transmission from the client. This channel does not have any agent-to-
 * client requests. The heartbeat should be sent at a 500ms interval
 *
...
// ========================================
// Decoding/Calculation Definitions
// ========================================
...
#define CANMORE_CONTROL_INTERFACE_MODE_NORMAL 0b001u
#define CANMORE_CONTROL_INTERFACE_MODE_BOOTLOADER 0b010u
#define CANMORE_CONTROL_INTERFACE_MODE_LINUX 0b011u
#define CANMORE_CONTROL_INTERFACE_MODE_BOOT_DELAY 0b101u
...
// Heartbeat Constants
#define CANMORE_HEARTBEAT_INTERVAL_MS 500
...
#define CANMORE_HEARTBEAT_MODE_LENGTH 3
#define CANMORE_HEARTBEAT_TERM_VALID_LENGTH 1
#define CANMORE_HEARTBEAT_TERM_ENABLED_LENGTH 1
...
#define CANMORE_HEARTBEAT_MODE_OFFSET CANMORE_HEARTBEAT_EXTRA_OFFSET
#define CANMORE_HEARTBEAT_TERM_VALID_OFFSET (CANMORE_HEARTBEAT_MODE_OFFSET + CANMORE_HEARTBEAT_MODE_LENGTH)
#define CANMORE_HEARTBEAT_TERM_ENABLED_OFFSET                                                                          \
    (CANMORE_HEARTBEAT_TERM_VALID_OFFSET + CANMORE_HEARTBEAT_TERM_VALID_LENGTH)

// Heartbeat Struct Definition
typedef union __attribute__((__packed__)) canmore_heartbeat {
    ...
    struct canmore_heartbeat_packet {
        ...
        uint8_t mode : CANMORE_HEARTBEAT_MODE_LENGTH;
        uint8_t term_valid : CANMORE_HEARTBEAT_TERM_VALID_LENGTH;
        uint8_t term_enabled : CANMORE_HEARTBEAT_TERM_ENABLED_LENGTH;
    } pkt;
} canmore_heartbeat_t;
...
#define CANMORE_CALC_TITAN_HEARTBEAT_DATA(cnt, error, mode, term_valid, term_enabled)                                  \
    ((((cnt) & ((1u << CANMORE_HEARTBEAT_CNT_LENGTH) - 1u)) << CANMORE_HEARTBEAT_CNT_OFFSET) |                         \
     ...
     (((mode) & ((1u << CANMORE_HEARTBEAT_MODE_LENGTH) - 1u)) << CANMORE_HEARTBEAT_MODE_OFFSET) |                      \
     (((term_valid) & ((1u << CANMORE_HEARTBEAT_TERM_VALID_LENGTH) - 1u)) << CANMORE_HEARTBEAT_TERM_VALID_OFFSET) |    \
     (((term_enabled) & ((1u << CANMORE_HEARTBEAT_TERM_ENABLED_LENGTH) - 1u))                                          \
      << CANMORE_HEARTBEAT_TERM_ENABLED_OFFSET))
