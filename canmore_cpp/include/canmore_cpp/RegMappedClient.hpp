...
#define REG_MAPPED_ERR_DEF(name)                                                                                       \
    ...
    static const std::string lookupMode(uint8_t mode) {
        if (mode == CANMORE_CONTROL_INTERFACE_MODE_NORMAL)
            return "Normal";
        else if (mode == CANMORE_CONTROL_INTERFACE_MODE_BOOT_DELAY)
            return "Boot Delay";
        else if (mode == CANMORE_CONTROL_INTERFACE_MODE_BOOTLOADER)
            return "Bootloader";
        else if (mode == CANMORE_CONTROL_INTERFACE_MODE_LINUX)
            return "Linux";
        else
