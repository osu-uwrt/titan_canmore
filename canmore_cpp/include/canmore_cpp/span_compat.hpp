...
#warning You are compiling with C++ 20. If you use std::span linker errors will occur!! \
    To fix this issue (hopefully the jetson isn not on Ubuntu 20 anymore), replace span_compat.hpp with <span> in canmore_cpp headers
#endif
...
public:
    ...
    // If this line is causing issues, sorry :(
    // This was originally defined as a pointer, but standard library stuff like find didn't like that
    // Instead, I just pulled the gnu stdc++ iterator definition. However, this isn't portable
    // If you're trying to compile this code on a mac or windows, and manager to get through the other
    // problems with using linux syscalls, then you should make this platform agnostic
#if __GNUC__
...
template <typename T, std::size_t N> TCB_SPAN_ARRAY_CONSTEXPR span<T, N> make_span(std::array<T, N> &arr) noexcept {
    return { arr };
}
...
template <typename Container> constexpr span<const typename Container::value_type> make_span(const Container &cont) {
    return { cont };
}
...
template <std::size_t N, typename E, std::size_t S> constexpr auto get(span<E, S> s) -> decltype(s[N]) {
    return s[N];
}
